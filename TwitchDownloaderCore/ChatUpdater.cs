...
namespace TwitchDownloaderCore
{
    public sealed class ChatUpdater
    {
        ...
        {
            List<TwitchEmote> firstPartyEmoteList = await KickHelper.GetEmotes(chatRoot.comments, _updateOptions.TempFolder, _updateOptions.ReplaceEmbeds ? null : chatRoot.embeddedData, cancellationToken: cancellationToken);
        ...
        {
            List<ChatBadge> badgeList = await KickHelper.GetChatBadges(chatRoot.comments, chatRoot.streamer.id, _updateOptions.TempFolder, _updateOptions.ReplaceEmbeds ? null : chatRoot.embeddedData, cancellationToken: cancellationToken);
        ...
        {
            List<CheerEmote> bitList = await KickHelper.GetBits(chatRoot.comments, _updateOptions.TempFolder, chatRoot.streamer.id.ToString(), _updateOptions.ReplaceEmbeds ? null : chatRoot.embeddedData, cancellationToken: cancellationToken);
